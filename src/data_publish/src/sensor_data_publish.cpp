...
// std::vector<ImuData> readImu(const std::string& filename, double time){
...
//         // }
//         // if (data.timestamp - first_time < time - 0.0000001) continue;
...
//                 // double s = 0.7071067811865476; // sin(45°)
//                 // double c = 0.7071067811865476; // cos(45°)
//                 // float qx1, qy1, qz1, qw1;
//                 // qx1 = 0.70710678f * (qx - qy);
//                 // qy1 = 0.70710678f * (qx + qy);
//                 // qz1 = 0.70710678f * (qz + qw);
//                 // qw1 = 0.70710678f * (qw - qz);
...
//                 // double s_x = -0.7071067811865476;  // sin(-45°)
//                 // double c_x =  0.7071067811865476;  // cos(-45°)

//                 // // 假设经过 Y 轴旋转后的四元数已经存储在 data 中
//                 // double p_x = data.x;
//                 // double p_y = data.y;
//                 // double p_z = data.z;
//                 // double p_w = data.w;

//                 // // 进行四元数乘法：data = data * q_x
//                 // data.x = p_x * c_x + p_w * s_x;      
//                 // data.y = p_y * c_x + p_z * s_x;      
//                 // data.z = p_z * c_x - p_y * s_x;     
//                 // data.w = p_w * c_x - p_x * s_x;     
...
//         data.w_y = r21 * w_x + r22 * w_y + r23 * w_z;
//         data.w_z = r31 * w_x + r32 * w_y + r33 * w_z;
...
// std::vector<VIOdata> readVIO(const std::string& filename, double first_time){
...
//             vio_msg.pose.covariance[0] = 0.02;//x
//             vio_msg.pose.covariance[7] = 0.02; // y
//             vio_msg.pose.covariance[14] = 0.02; // z

//             vio_msg.pose.covariance[21] = 0.2; // roll
//             vio_msg.pose.covariance[28] = 0.2; // pitch
//             vio_msg.pose.covariance[35] = 0.2; // yaw
...
//     double first_time;
//     double time = 0;
//     double intial_gps_longtitude;
//     double intial_gps_latitude;
//     bool if_use_intial_gps = false;
...
//     nh.getParam("if_use_intial_gps", if_use_intial_gps);
//     if (if_use_intial_gps){
//         nh.getParam("intial_gps_longtitude", intial_gps_longtitude);
//         nh.getParam("intial_gps_latitude", intial_gps_latitude);
//     }




//     std::vector<ImuData> imu_data = readImu(imu_csv_file, time);
//     first_time = imu_data[0].timestamp;
//     std::vector<VIOdata> vio_data = readVIO(vio_csv_file, first_time);
//     std::vector<GpsData> gps_data = readGps(gps_csv_file, first_time);

//     if (if_use_intial_gps){
//         gps_data[0].longitude = intial_gps_longtitude;
//         gps_data[0].latitude = intial_gps_latitude;
//     }
...
// Holds IMU data (orientation, angular velocity, and linear acceleration).
struct ImuData{
    double timestamp;
    float x, y, z, w;
    float w_x, w_y, w_z;
...
// Holds GPS data (lat, lon, alt, etc.).
struct GpsData{
    double timestamp;
    double latitude, longitude, altitude;
    float horacc, veracc;
...
// Holds VIO data (timestamp, frame, position, orientation).
struct VIOdata{
    double timestamp;
    int frame;
    float px, py, pz;
...
// Finds the index of the timestamp closest to target_time.
template<typename T>
size_t findClosestTimeIndex(const std::vector<T>& data, double target_time) {
    auto it = std::min_element(data.begin(), data.end(), [target_time](const T& a, const T& b) {
        return std::abs(a.timestamp - target_time) < std::abs(b.timestamp - target_time);
...
// Another way of quaternion multiplication with individual float parameters.
void quaternionMultiply0(float w1, float x1, float y1, float z1,
                        float w2, float x2, float y2, float z2,
                        float &w, float &x, float &y, float &z) {
    w = w1 * w2 - x1 * x2 - y1 * y2 - z1 * z2;
...
// Reads IMU CSV and transforms orientation, angular velocity, and acceleration.
std::vector<ImuData> readImu(const std::string& filename, double time){
    std::vector<ImuData> imu_data;
    std::fstream file(filename);
    std::string line;
    ...
    while (std::getline(file, line)){
        ...
        // Simple quaternion transform (example).
        data.x = 0.70710678f * (qx - qy);
        data.y = 0.70710678f * (qx + qy);
        data.z = 0.70710678f * (qz + qw);
        data.w = 0.70710678f * (qw - qz);

        // Rotation matrix (simplified example) to transform acc and gyro to world frame.
        double r11 = 1.0 - 2.0 * (qy * qy + qz * qz);
        double r12 = 2.0 * (qx * qy - qz * qw);
        double r13 = 2.0 * (qx * qz + qy * qw);
...
// Reads GPS CSV and skips entries before the first IMU time.
std::vector<GpsData> readGps(const std::string& filename, double first_time){
    std::vector<GpsData> gps_data;
    std::fstream file(filename);
    std::string line;
...
// Reads VIO CSV and applies transformations to orientation.
std::vector<VIOdata> readVIO(const std::string& filename, double first_time){
    std::vector<VIOdata> vio_data;
    std::fstream file(filename);
    std::string line;
    ...
    while(std::getline(file, line)){
        ...
        // B->A, A->C, C->D transformations as an example.
        tf2::Quaternion quat_A_to_C(x, y, z, w);
        tf2::Quaternion quat_C_to_D;
        tf2::Quaternion rotation_y, rotation_z;
        rotation_y.setRPY(0, tf2Radians(180), 0);
        rotation_z.setRPY(0, 0, tf2Radians(-90));
        quat_C_to_D = rotation_y * rotation_z;

        tf2::Quaternion quat_B_to_A;
        quat_B_to_A.setRPY(tf2Radians(90), 0, 0);
...
// Publishes IMU, GPS, and VIO data in chronological order.
void publishData (const std::vector<ImuData>& imu_data, const std::vector<GpsData>& gps_data,
                  const std::vector<VIOdata>& vio_data){
    ros::NodeHandle nh;
    ros::Publisher imu_pub = nh.advertise<sensor_msgs::Imu>("imu_data", 10);
    ...
    while(ros::ok() && (imu_index < imu_data_size || gps_index < gps_data_size || vio_index < vio_data_size)){
        ...
        double imu_wait = imu_index < imu_data_size ?
            (imu_data[imu_index].timestamp - global_start_time) - (current_time - ros_start_time) :
            std::numeric_limits<double>::max();
        double gps_wait = gps_index < gps_data_size ?
            (gps_data[gps_index].timestamp - global_start_time) - (current_time - ros_start_time) :
            std::numeric_limits<double>::max();
        double vio_wait = vio_index < vio_data_size ?
            (vio_data[vio_index].timestamp - global_start_time) - (current_time - ros_start_time) :
            std::numeric_limits<double>::max();

        // Publish IMU if it's time
        if (imu_index < imu_data_size && imu_wait <= 0){
            sensor_msgs::Imu imu_msg;
            nav_msgs::Odometry orientation;
        ...
        if (vio_index < vio_data_size && vio_wait <= 0){
            ...
            vio_msg.pose.covariance[0] = 0.02;
            vio_msg.pose.covariance[7] = 0.02;
            vio_msg.pose.covariance[14] = 0.02;
            vio_msg.pose.covariance[21] = 0.2;
            vio_msg.pose.covariance[28] = 0.2;
            vio_msg.pose.covariance[35] = 0.2;
...
// Main entry: loads parameters, reads data, and starts publishing.
int main(int argc, char** argv){
    ros::init(argc, argv, "sensor_data_publish");
    ros::NodeHandle nh("~");
    std::string imu_csv_file;
